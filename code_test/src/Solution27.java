 ...
 */
    public int solution(int[] d, int budget) {
        ...
        // sort()메소드를 이용하여 d 배열을 오름차순으로 정렬(작은 수들을 먼저 더해야 많은 부서에게 분배 가능하기 때문)
        Arrays.sort(d);

        for (int i = 0; i < d.length; i++) {
            budget -= d[i]; // 전체 예산에서 각 부서의 지원금을 빼낸다
            if (budget < 0) break; // 전체 예산이 0보다 작아지면 break
            answer++;
        }
        return answer;
    }
    ...
    public static void main(String[] args) {
        ...
        int[] d = {1,3,2,5,4};
        int budget = 9;
        System.out.println("결과: " + sol.solution(d, budget));
    }
}
